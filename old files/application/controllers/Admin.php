    ...
    public function get_class_types(){
      ...
      if(!empty($class_levels)){
        ...
        $class_levels = explode(',',$class_levels);
          ...
          $list .= '<option value="'.$level.'">'.$this->Admin_model->get_type_name_by_id('standard_master','id',$level,'standard_name').'</option> '; 
...
if(is_uploaded_file($_FILES['product_image']['tmp_name'])) {
 ...
 $data['standard'] = array('id'=>$standard->id,'standard_name'=>$standard->standard_name,'type_id'=>$standard->type_id);
  ...
  {  
   $session = $this->session->userdata('superadmindet');
    ...
    $data['schooldet'] = $this->Admin_model->get_all_data('school_master',array('status !='=>'C')) ;
  ...
  public function  viewuniforms(){
   ...
   foreach ($uniforms as $value){
    $levels = explode(',', $value['class_level']) ;
      ...
      foreach ($levels as $lev) {
        $classArray[] =  $lev ;
        //$this->Admin_model->get_type_name_by_id('standard_master','id',$lev,'standard_name')  ;
      }
   ...
   foreach($classArray as $class){
    $get_uniforms  = $this->Admin_model->get_all_data('uniform_school_relation',array('school_id'=>$schoolId,'status'=>1 , 'find_in_set("'.$class.'", class_level) <> 0'));

    $key = array_column($get_uniforms, 'class_level');
    array_multisort($key, SORT_ASC, $get_uniforms) ;

    foreach($get_uniforms  as $uniform){
      $Uvalues[] = array('class' => $class, 'unformId'=>$uniform['uniform_id']);
    }
 ...
 public function newschool()
  {   
    ...
    $data['levels'] =  $this->Admin_model->get_all_data('standard_master');
  ...
  $fname = "";
    ...
    'class_levels' => implode(',',$this->input->post('class_levels')),
 ...
 $data = array();
if (isset($school)){
    $data  = array('id'=>$school->id,'school_name'=>$school->school_name,'ar_school_name'=>$school->ar_school_name,'class_levels'=>$school->class_levels,'school_type'=>$school->school_type, 'school_logo'=>$school->school_logo,'description' => $school->description,'ar_description' => $school->ar_description);
}
...
$getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
 ...
 if(!empty($this->input->post('measurements'))){
  ...
  $measurements= '' ;
    ...
    $result = $this->Admin_model->insert_data('school_products', $data);
    if ($result == TRUE) {
...
public function question_status()
  ...
  $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
      ...
      $this->Admin_model->update_data('questions',array('id'=>$id),array('status'=>$status));
...
'status' => 'deleted'
 ...
 $result = $this->Admin_model->update_data('questions',array('id'=>$questionid), $data);
 

 //echo $this->db->last_query();
    if ($result == TRUE) {
        $Return['result'] = $this->Admin_model->translate('Question deleted successfully');
...
$users = $this->Admin_model->get_single_data('users',array('id'=>$id));
 ...
 $data = array();
if (isset($users)){
    $data  = array('id'=>$users->id,'user_name'=>$users->user_name,'email'=>$users->user_email,'phone'=>$users->phone);
}

$newrequest = $this->Admin_model->get_all_data('password_request',array('status'=>'new','user_id'=>$id)) ;
   // echo $this->db->last_query();
    $data['requests'] = $this->Admin_model->get_all_data('password_request',array('status'=>'new')) ;

    $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
        $this->load->view('admin/changerequests', $data);
    }else{

    if($newrequest){
$data['requested'] = $this->Admin_model->translate('Your request for password change is pending. You cannot create multiple requests') ;
    } else{
$data['requested'] = "" ;
    }
 
     $this->load->view('admin/requestnewpassword', $data);
 ...
 public function newuser()
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    }else if(!empty($this->Admin_model->get_single_data('users',array('email'=>$this->input->post('email'))))){
...
public function reset_pass()
  ...
  $id =$this->input->post('userid') ;
    ...
    public function requests()
   {
     ...
     $data['serviceid']=$this->input->get('id');
...
if($todate){
 ...
 public function  adduniformtoschool(){
    ...
    $data['schools'] = $this->Admin_model->get_all_data('school_master',array('status !='=>'C'));
  ...
  {  
   $session = $this->session->userdata('superadmindet');
    if(empty($session)){ 
     ...
     $data['disabilities'] = $this->Admin_model->get_all_data('disabilities',''); 
 

$getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');

    if( $getval){
    $this->load->view('admin/list_disability', $data);
    }else{
      redirect('admin/accessdenied');
    }
 ...
 public function newdisability()
  {   
    ...
    if( $getval){
    $this->load->view('admin/add_disability');
    }else{
      redirect('admin/accessdenied');
    }
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    /* Server side PHP input validation */    
    if($this->input->post('disability_name')==='') {
          $Return['error'] = "Add disability name ";
    }else if($this->input->post('ar_disability_name')==='') {
          $Return['error'] = "Add disability name in arabic";
    }else if($this->input->post('dis_group')==='') {
          $Return['error'] = "Add disability group";
    }   
  ...
  if($Return['error']!=''){
    ...
    'disability_name' => $this->input->post('disability_name'),
     'ar_disability_name' => $this->input->post('ar_disability_name'),
    'disability_group'=>$this->input->post('dis_group'),
     
      );
    $result = $this->Admin_model->insert_data('disabilities', $data);
    if ($result == TRUE) {
      
      //get setting info 
       
       $Return['result'] = 'Disability added successfully.';
 ...
 $id = $this->uri->segment(3) ;
 $modules = $this->Admin_model->get_single_data('disabilities',array('id'=>$id));

 $data = array();
if (isset($modules)){
    $data  = array('id'=>$modules->id,'disability_name'=>$modules->disability_name,'ar_disability_name'=>$modules->ar_disability_name, 'dis_group'=>$modules->disability_group);
}
   $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');

    if( $getval){
    $this->load->view('admin/edit_disability', $data);
  ...
  $id =$this->input->post('disabilityid') ;
    ...
    $result = $this->Admin_model->update_data('disabilities',array('id'=>$id), $data);
    if ($result == TRUE) {
      
      //get setting info 
       
       $Return['result'] = 'Disability updated successfully.';
  ...
  {  
   $session = $this->session->userdata('superadmindet');
    ...
    $this->db->select('count(id)count, service_id');
    $this->db->from('service_request');
     $this->db->where('status','new');
    $this->db->group_by('service_id');

    $data['request_det'] = $this->db->get()->result_array(); 

    $data['totalRequest'] =  count($this->db->get_where('service_request',array('status'=>'new'))->result_array());


    $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');

    if( $getval){
       $this->load->view('admin/open_issues', $data);
    }else{
      redirect('admin/accessdenied');
    }
...
if($status){
 ...
 $orderby = ' created_on desc ' ;
     ...
     public function guests()
  {
    ...
    $data['custdet'] = $this->Admin_model->get_all_data('customers',array('type'=>'guest')) ;
  ...
  public function reportrequests()
  {
    ...
    $data['custdet'] = $this->Admin_model->get_all_data('report_requests') ;
    
    $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
       $this->load->view('admin/listreprequests', $data);
    }else{
      redirect('admin/accessdenied');
    }
...
public function reportprocessed()
{
  $session = $this->session->userdata('superadmindet');
    ...
    if( $status=='new'){
      ...
      $newstatus="new" ;
        ...
        $this->db->where('id',$id);
        $this->db->update('report_requests',array('status'=>$newstatus));
}
  ...
  {  
   $session = $this->session->userdata('superadmindet');
    ...
    $data['requestdet'] = $this->Admin_model->get_all_data('service_request','') ;
    $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if($getval){
...
public function viewdet()
  {  
   $session = $this->session->userdata('superadmindet');
    ...
    $data['reqdata'] = $this->Admin_model->get_single_data('service_request',array('id'=> $requestid)) ;
    
    $data['requestId'] =  $requestid ;
    // $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    // if($getval){
    //  $this->load->view('admin/viewrequestdet', $data);
    // }else{
    //   redirect('admin/accessdenied');
    // }
$this->load->view('admin/viewrequestdet', $data);
 ...
 public function listfaq()
  {  
   $session = $this->session->userdata('superadmindet');
    ...
    $data['faqdet'] = $this->Admin_model->get_all_data('faq',''); 
...
if( $getval){
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    if($this->input->post('question')==='') {
          $Return['error'] = "Add Question ";
    }else if($this->input->post('answer')==='') {
          $Return['error'] = "Add Answer ";
    }
    ...
    $result = $this->Admin_model->insert_data('faq', $data);
    if ($result == TRUE) {
      
      //get setting info 
       
       $Return['result'] = 'FAQ added successfully.';
 ...
 $id = $this->uri->segment(3) ;
 $service = $this->Admin_model->get_single_data('faq',array('id'=>$id)) ;
 
 $data = array();
if (isset($service)){
    $data  = array('id'=>$service->id,'que'=>$service->question,'ar_que'=>$service->ar_question,'answer'=>$service->answer,'ar_answer'=>$service->ar_answer);
}

 $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin','admin');
    if( $getval){
...
$this->db->where('id',$faqid);
 ...
 //echo $this->db->last_query();
    if ($result == TRUE) {
        $Return['result'] = 'Question deleted successfully.';
...
// $result = $this->db->update('faq',$data);
    $result = $this->Admin_model->update_data('faq',array('id'=>$id), $data);
    if ($result == TRUE) {
      
      //get setting info 
       
       $Return['result'] = 'faq updated successfully.';
...
public function faq_status()
  ...
  $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
     ...
     $this->Admin_model->update_data('faq',array('id'=>$id),array('status'=>$status));
...
$session = $this->session->userdata('superadmindet');
 ...
 $master = $this->Admin_model->get_single_data('masters',array('id'=>'1')) ;
if(!empty($master))
{

    $data  = array('mobile'=>$master->mobile,'mail'=>$master->mail);
}
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    if($this->input->post('mobile')==='') {
          $Return['error'] = "Add Mobile Number";
    }else if($this->input->post('mail')==='') {
          $Return['error'] = "Add mail id ";
    }
    ...
    $result = $this->Admin_model->update_data('masters',array('id'=>1), $data);
    if ($result == TRUE) {
...
$session = $this->session->userdata('superadmindet');
 ...
 $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
  $this->load->view('admin/edit_brief', $data);
    }else{
      redirect('admin/accessdenied');
    }
...
$data['briefdet'] = $this->Admin_model->get_single_data('brief',array('id'=>'1')) ;
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    }else if($this->input->post('basic')==='') {
          $Return['error'] = "Content Required ";
    }else if($this->input->post('ar_basic')==='') {
          $Return['error'] = "Arabic Content Required ";
    }else if($this->input->post('address')==='') {
          $Return['error'] = "Address Required ";
    }else if($this->input->post('ar_address')==='') {
          $Return['error'] = "Address in arabic Required ";
    }else if($this->input->post('langlat')==='') {
          $Return['error'] = "Select Location ";
    }
    // else if(!file_exists($_FILES['service_icon']['tmp_name']) || !is_uploaded_file($_FILES['service_icon']['tmp_name'])) {
    //       $Return['error'] = "Icon required";
    // }else if(!file_exists($_FILES['video']['tmp_name']) || !is_uploaded_file($_FILES['video']['tmp_name'])) {
    ...
    if($Return['error']!=''){
      ...
      if(in_array($ext,$allowed)){
        ...
        // basename() may prevent filesystem traversal attacks;
        // further validation/sanitation of the filename may be appropriate
        $name = basename($_FILES["service_icon"]["name"]);
       // $newfilename = 'cat_'.round(microtime(true)).'.'.$ext;
  ...
  //      $fname2 =  $this->input->post('oldvideo');
  ...
  //       $set_img = base_url()."uploads/video/";
  //       // basename() may prevent filesystem traversal attacks;
  //       // further validation/sanitation of the filename may be appropriate
  ...
  //   }
  // }
      $brief = array('title'=>$this->input->post('title'),'ar_title'=>$this->input->post('ar_title'),'content'=>$this->input->post('basic'),'ar_content'=>$this->input->post('ar_basic'),'logo'=>$fname);
      
      $briefval = json_encode($brief);
    $data = array(   
    'address' => $this->input->post('address'),
      'ar_address' => $this->input->post('ar_address'),
    'location' => $this->input->post('langlat'),
   // 'video' =>  $fname2,
    'brief' => $briefval,
    //  'description' => $this->input->post('desc'),
    //   'ar_description' => $this->input->post('ar_desc'),
       'url' => $this->input->post('url'),
   
      );
    //print_r($data);
    $result = $this->Admin_model->update_data('brief',array('id'=>1), $data);
    if ($result == TRUE) {
...
public function sup_org()
{
  $session = $this->session->userdata('superadmindet');
    ...
    $data['faqdet'] = $this->Admin_model->get_all_data('supp_org',''); 

    $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');

if( $getval){
    $this->load->view('admin/list_supporg', $data);
}else{
  redirect('admin/accessdenied'); 
}
...
public function supporg_status()
  {  
   $session = $this->session->userdata('superadmindet');
    if(empty($session)){ 
      redirect('admin');
  ...
  $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
     ...
     $this->Admin_model->update_data('supp_org',array('id'=>$id),array('status'=>$status)); 
     return $id ; 
  ...
  public function new_supp_org()
  {
   ...
   $session = $this->session->userdata('superadmindet');
    if(empty($session)){ 
     ...
     $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
    if( $getval){
    $this->load->view('admin/add_supporg');
    }else{
      redirect('admin/accessdenied');
    }
     // $this->load->view('admin/add_supporg');
  ...
  {   
   ...
   $Return = array('result'=>'', 'error'=>'');
    ...
    if($this->input->post('title')==='') {
          ...
          $Return['error'] = "Add arabic title ";
    }else if(empty($_FILES['files']['name'])){
      $Return['error'] = "Icon Required ";
    }
  ...
  $img = [];
  $icons = array(); 
      $count = count($this->input->post('org_name'));

    $orgname =  $this->input->post('org_name') ;
    $ar_orgname =  $this->input->post('ar_org_name') ;
    $weburl =  $this->input->post('website_url') ;
   ...
   for($i=0;$i<$count;$i++){
    ...
    $url =  $weburl[$i] ;
        ...
        if(empty($filename)){
           $Return['error'] = "Icon cannot be empty";
        }else if($org == ''){
            $Return['error'] = "Organization name in english required";
        }else if($ar_org==''){
            $Return['error'] = "Organization name in arabic required";
        } 
      ...
      for($i=0;$i<$count;$i++){
        ...
        if(!empty($_FILES['files']['name'][$i])){
          ...
          if($this->upload->do_upload('file')){
            ...
            $img['totalFiles'][] = $filename;
          }        
        }

       $organizations[] = array('orgIcon'=>$filename,'orgname'=>$orgname[$i] ,'ar_orgname'=>$ar_orgname[$i],'weburl'=>$weburl[$i]) ;
   
      }

   // print_r($data['totalFiles']);

$icons = implode(',', $img['totalFiles']);
  ...
  //  $result = $this->Admin_model->insert_data('supp_org', $data);
    $result = $this->db->insert('supp_org',$data);
    ...
    $session = $this->session->userdata('superadmindet');
        if(empty($session)){ 
          redirect('admin');
            ...
            $id = $this->uri->segment(3) ;
     $data['suppdet'] = $this->Admin_model->get_single_data('supp_org',array('id'=>$id)) ;

     $getval = $this->checkaccess($session['userrole'] ,$this->router->fetch_method(),'admin');
        if( $getval){
      $this->load->view('admin/edit_suporg', $data);
        }else{
          redirect('admin/accessdenied');
        }
