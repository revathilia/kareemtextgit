...
<div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">  Update Enquiry Status</h4>
      </div>


 <div class="modal-body"> 
<form class="order_form" id="status_form" method="POST" action="<?php echo base_url();?>admin/enquiry_status_update">

  <input type="hidden" name="enquiryid" id="service" value="<?php echo $enquiryid ?>">

<div class="form-group">
<label class="control-label mb-10 text-left"><?php echo $this->Admin_model->translate("Remarks") ?> </label>
 
<textarea placeholder="Add remarks here...." name="remarks" class="form-control" > <?php echo $reqstatus->remarks ?></textarea>
</div>
...
<select class="form-control" id="field_type" name="status"><option value="">--Select--</option>
   <?php foreach ($statuses as  $value) {?>
    <option value="<?php echo $value['id'] ?>" <?php if($reqstatus->status == $value['id']) { echo 'selected' ; } ?>><?php echo $value['status'] ?></option>
  <?php } ?>
</select>
</div>
</div>
